...
class OperationTableViewController: UITableViewController, NSFetchedResultsControllerDelegate, OperationEditorViewControllerDelegate
{
	...
	/*	This function was implemented to handle the case where an operation has multiple "resection" types, thus requiring
		one cell to contain multiple lines and be taller than the default height. */
	override func tableView(tableView: UITableView, heightForRowAtIndexPath indexPath: NSIndexPath) -> CGFloat
	{
		if indexPath.section == 0 && indexPath.row == 2 && operation!.resectionsArray().count > 0
		{
			// calculate custom height for 'resection' cell
